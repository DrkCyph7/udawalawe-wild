...
function DetailPanel({
  ...
  const PARTNERS = [
    ...
    "Eco Safari Sri Lanka",
    "Elephant Transit Home Tours",
    "Wilderness Edge Safaris",
  ];

  const savePartner = async (newPartner: string) => {
    if (!supabase) return;
    ...
    try {
      const { error } = await supabase
        ...
        .update({ assigned_partner: newPartner === "Unassigned" ? null : newPartner })
        .eq("id", row.id);
      if (error) throw error;
      toast("Partner updated.");
      // Also update the row in parent state if we had a callback for it, but for now it will just update locally in the panel.
    } catch {
      toast("Failed to assign partner.", "err");
      setPartner(row.assigned_partner ?? ""); // revert
    } finally {
          ...
          <div className="rounded-xl border border-border bg-background p-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  value={partner || "Unassigned"}
                  onChange={(e) => void savePartner(e.target.value)}
                  disabled={savingPartner}
                  className="w-full rounded-md border border-input bg-background py-1.5 pl-2 pr-6 text-sm focus:outline-none focus:ring-2 focus:ring-ring disabled:opacity-50"
                >
                  {PARTNERS.map((p) => (
...
export default function AdminDashboard({
  ...
  useEffect(() => {
    async function init() {
      try {
        const rows = await fetchBookingsForRole(initialRole);
